...
using namespace std; // Pra facilitar o trabalho kkk
...
int main(){ // Vai executar o programa
    ...
    string endmod[256]; // endereços que foram modificados, para não ter repetição
    endmod[0] = "nada"; // endereço inicial começa com "nada"
    ...
    while(true){ //cout << memoria[i] << endl;
        ...
        if(rdm == "00"){ // Execução da instrução NOP
            ...
            if(endmod[0] == "nada"){
                ...
                bool alterou = false;
                for(int i = 0; i <= numeroend; i++) // procura se já foi modificado esse endereço
                {
                    if(endmod[i] == ende){ // se for substitui
                        enderecos[i] = ende + " " + AC;
                        alterou = true;
                    }
                }
                if(!alterou){ // se não foi agora ele é
                    enderecos[++numeroend] = ende + " " + AC;
                    endmod[numeroend] = ende;
                }
            }
            ///enderecos[++numeroend] = ende + " " + AC;
